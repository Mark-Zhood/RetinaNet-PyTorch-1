...
from utils.box_tools import NMS
import numpy as np
...
def Eval(model, test_dataset):
    ...
    model.eval()
    test_loader = DataLoader(dataset=test_dataset, batch_size=cfg.batch_size, num_workers=cfg.num_workers,shuffle=False)
    with torch.no_grad():
        for images, boxes, labels, image_names in tqdm(test_loader):
            images = images.cuda()
            pred_cls, pred_loc = model(images)
            results = NMS(pred_cls, pred_loc)
            for image_name, result in zip(image_names, results):
                ...
                gt_boxs, gt_labels, w, h = test_dataset._get_annotation(image_name)
                # 根据原始输入图像尺寸转换成绝对坐标
                pred_boxes[:, 0::2] *= w
                pred_boxes[:, 1::2] *= h
    ...
    result = calc_pr(pred_boxes_list, pred_labels_list, pred_scores_list, gt_boxs_list, gt_labels_list)
    return result
...
def calc_pr(pred_boxes, pred_labels, pred_scores, gt_boxes, gt_labels):
    ...
    for pred_box, pred_label, pred_score, gt_box, gt_label in zip(pred_boxes, pred_labels, pred_scores, gt_boxes,gt_labels):
        ...
        for l in np.unique(gt_label):
            ...
            if len(pred_box_l) == 0:
                continue
            iou = box_iou(pred_box_l[:, None], gt_box_l[None, :],is_tensor=False)
