...
with torch.no_grad():
    # 开始读取视频源或摄像头
    vid = cv2.VideoCapture(cfg.video_path)
    ...
    while True:
        ...
        if return_value:
            ...
            pred_boxes, pred_labels, pred_scores = NMS(pred_cls, pred_locs)[0]
            pred_boxes[:, 0::2] *= w
            pred_boxes[:, 1::2] *= h
            end_time = time.time()
            # FPS计算方式比较简单
            ...
            # 加载字体文件
            font = ImageFont.truetype(font='FiraMono-Medium.otf',size=16)
            # 目标框的厚度
